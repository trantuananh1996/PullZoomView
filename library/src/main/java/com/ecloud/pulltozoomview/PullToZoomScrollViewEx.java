...
public class PullToZoomScrollViewEx extends PullToZoomBase<NestedScrollView> {
    ...
    private int minHeaderHeight = -1;
    private ScalingRunnable mScalingRunnable;
    private OnScrollHeightChanged onScrollHeightChanged;
    private static final Interpolator sInterpolator = paramAnonymousFloat -> {
        float f = paramAnonymousFloat - 1.0F;
        return 1.0F + f * (f * (f * (f * f)));
    };

    public void setOnScrollHeightChanged(OnScrollHeightChanged onScrollHeightChanged) {
        this.onScrollHeightChanged = onScrollHeightChanged;
    }
    ...
    public PullToZoomScrollViewEx(Context context, AttributeSet attrs) {
        ...
        ((InternalScrollView) mRootView).setOnScrollViewChangedListener((left, top, oldLeft, oldTop) -> {
            if (isPullToZoomEnabled() && isParallax()) {
                ...
                if (onScrollHeightChanged != null) onScrollHeightChanged.onScrollHeightChanged(f);
            }
        });
    }
    ...
    protected void pullHeaderToZoom(int newScrollValue) {
        d(TAG, "pullHeaderToZoom --> newScrollValue = " + newScrollValue);
        d(TAG, "pullHeaderToZoom --> mHeaderHeight = " + mHeaderHeight);
...
//            LayoutInflater mLayoutInflater = LayoutInflater.from(getContext());
            mContentView = View.inflate(getContext(),contentViewResId,null);
//            mLayoutInflater.inflate(contentViewResId, null, false);
        }
    ...
    public interface OnScrollHeightChanged {
        void onScrollHeightChanged(float height);
    }
}
